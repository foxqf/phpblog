...
function resize(obj,jd){
    var evt = getEvent();
      ...
      document.onmouseup = function () {
        ...
        $(".lt-content").height($(".lt-dlg-box").height()-83);
      };
}

function CreateWindows(title, msg, w, h,callback){



    var msgObj=$('<div class="lt-dlg-box" style="display:none;z-index:'+(zindex++)+';top:'+($(window).height()-h)/2+'px;left:'+($(window).width()-w)/2+'px;width:'+w+'px;height:'+h+'px;"></div>');
    $('body').append(msgObj);
    var dv = $('<div class="modal-backdrop1" style="display:none"></div>');
    dv.fadeTo('show',0.5);
    msgObj.fadeIn(100,callback);
    $('body').append(dv);
    dv.click(function(){
        //msgObj.remove();
        //dv.remove();
  ...
  var closeBtn = $('<span class="hy-r" style="font-size:15pt; color:#000;cursor: pointer;">×</span>')
    ...
    var msgBox = $('<div class="lt-content" style="overflow: auto;">'+msg+'</div>');
    msgBox.height(height - 54);
...
//打开对话框  发表主题
function show_post_thread(){
    var width=700;height=750;
    if($(window).width() < width)   width  = $(window).width() - 20;
    if($(window).height() < height) height = $(window).height() - 20;

    CreateWindows("发表主题",'编辑器正在加载中',width,height,function(){
        $('.lt-content').load(www+'ajax'+exp+'thread');
        $(".lt-content").height($(".lt-dlg-box").height()-83);
    });
...
function show_edit_post(id){
    ...
    CreateWindows("编辑帖子",'编辑器正在加载中',width,height,function(){
        $('.lt-content').load(www+'ajax'+exp+'editpost?id='+id);
        $(".lt-content").height($(".lt-dlg-box").height()-83);
    });
}

function show_post_post(id){
    var width=600;height=520;
    if($(window).width() < width)   width  = $(window).width() - 20;
    if($(window).height() < height) height = $(window).height() - 20;

    CreateWindows("回复帖子",'编辑器正在加载中',width,height,function(){
        $('.lt-content').load(www+'ajax'+exp+'post?id='+id);
        $(".lt-content").height($(".lt-dlg-box").height()-83);
    });
}
